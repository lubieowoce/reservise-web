export const scoreIndicator = ({numPeople, numScores, size=10, style={}}) => {
	const fill = (
		numScores === 0 ? 'white' : 'rgba(0,0,0,0)'
	)
	const [borderColor, borderWidth] = (
		numPeople === 0 ? ['rgba(0,0,0, 0.15)', '1px'] :
		numScores === 0 ? ['currentColor', '2px'] :
		['rgba(0,0,0,0)', '0px']
	)
    return $(`
        <svg height="${size+4}" width="${size+4}">
		  <polygon points="0,0 ${size},0 0,${size}" style="fill: ${fill}; stroke-width: 0" />
		  <line x1=0 y1=${size+1} x2=${size+1} y2=0 style="stroke:${borderColor}; stroke-width:${borderWidth}" />
		</svg>
    `).css(style)
}
