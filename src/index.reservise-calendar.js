...
import { is_nonempty, is_instance_of, group_by } from "./utils";
...
const popover_annotation_node = (popover) => {
  ...
  let ann_wrapper = event_details.querySelector(
    "#popoverReservationAnnotation li:not(.custom-wrapper)"
  );
  return ann_wrapper.querySelector(
    ".smartform-enabled, .smartform-disabled, .smartform-static"
  );
};
...
const is_single_reservation = (event) =>
  !is_class_reservation(event) && !is_unavailability(event);
...
const patchSmartFormInput = ($annotationNode) => {
  let [annotationText, annotationData] = annotations.parse(
    $annotationNode.val()
  );
  ...
  ClientDataWrapper = class {
    ...
    handleChange({ user_entries, game_results }) {
      ...
      if (changed) {
        ...
        setTimeout(
          () =>
            this.render().then(() => this.onChange(stateToData(this.state))),
          0
        );
      }
    }
...
window.ReservationDetailsPopover = function (
  event_id,
  refetch_func,
  attachment_func
) {
  console.log("intercepted!", event_id);
  const self = new ORIGINAL.ReservationDetailsPopover(
    event_id,
    refetch_func,
    attachment_func
  );
  ...
  self.afterReservationDetailsRender = () => {
    ...
    if (!shouldSkip) {
      const $annotationNode = $(
        get_event_details_uninited_annotation_node(popoverNode)
      );
    ...
    if (!shouldSkip) {
      ...
      const annotationSmartform = $(popover_annotation_node(self)).data(
        "bs.smartform"
      );
      annotationSmartform.inject_custom_data = inject_custom_data;
      annotationSmartform.get_custom_data = () => widget.getData();
      annotationSmartform.$element.on(
        "fetchsuccess.bs.smartform",
        function (_evt, data) {
          const annData = patchSmartFormInput(
            annotationSmartform.inputElements()
          );
  ...
  self.showPopover = function () {
    ...
    popover.addEventListener("keyup", function (evt) {
      if (evt.key === "Escape") {
        if (
          event.target.matches('input:not([type="button"]), textarea, select')
        ) {
...
const reservation_owner_from_popover = ($popover) => {
  ...
  const [link] = $popover
    .find('.detailsHeader a[href^="/clients/c/"]')
    .toArray();
  if (!link) {
    return null;
  }
  ...
  const [phone_icon] = $popover
    .find(".detailsHeader .glyphicon-phone")
    .toArray();
  if (phone_icon) {
    const phone =
      phone_icon.getAttribute("title") ||
      phone_icon.getAttribute("data-original-title");
    if (phone) {
      label += ` (${phone})`;
    }
  }
...
const renderSidebarComponents = () => {
  ...
  const reservationsToday = Object.values(window.calendar.reservationsById)
    .filter(
      (r) =>
        r !== undefined &&
        r.event !== undefined &&
  ...
  const userEntriesWithCard = reservationsToday.flatMap((r) => {
    ...
    <CardList
      ...
      onSyncCards={async () => {
        ...
        if (result.success) {
          ...
          ui.show_error(
            `Problem z synchronizacją kart zniżkowych: ${reason} (dodano ${num_added})`
          );
        }
  ...
  for (const r of reservationsToday) {
    const event = r.event;
    const data = r.data;
    if (!data || !data.users || !data.game_results) {
      continue;
    }
...
window.BaseReservationEvent.prototype.render = function (event, element) {
  ...
  if (
    is_instance_of(event, window.ReservationStub) ||
    event.className.includes("rsv-stub")
  ) {
  ...
  const { users: user_entries = [], game_results: scores = [] } = annotation
...
export async function sync_card_annotations_with_reservation({
  user_entries,
  dry_run = true,
}) {
  ...
  if (!benefit_res) {
    return {
      success: false,
      num_added: 0,
      reason: 'Nie znaleziono zajęć grupowych o nazwie "Karty zniżkowe"',
    };
  }
  const benefit_res_id = benefit_res.id;
  const existing = (await api.class_event_reservations(benefit_res_id)).filter(
    (r) => r.status === "active"
  );
  ...
  for (const [client_id, entries] of Object.entries(ann_ms_by_client)) {
    ...
    for (let i = 0; i < diff; i++) {
      ...
      const res = await ui.add_benefit_reservation({
        client_id,
        benefit_res_id,
      });
      console.log(res);
...
$(document).ready(() => {
  const head = $("head");
  head.append(
    $('<style id="card-info-badge-styles">').text(card_count_badge.style)
  );
