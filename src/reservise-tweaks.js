...
async function edit_popup_price() {
  ...
  if (popup === null || !is_reservation_popover(popup)) {
    return null;
  }
...
async function add_benefit_card() {
  ...
  if (window.calendar.HAS_ADD_CLIENTS === true) {
    ...
    let custom_user_search = popup.querySelector(
      ".custom-wrapper .custom-add-user-search"
    );
    if (custom_user_search === null) {
      return;
    }
...
function search_selector(search_start, selector, search_direction) {
  ...
  if (search_direction === "up") {
    ...
    return getp(
      [...search_start.querySelectorAll(selector)].filter(is_visible),
      0,
      null
    );
  } else {
...
function close_current_popup() {
  ...
  let popup = find_current_popup();
  if (popup !== null) {
    console.log("close_current_popup :: closing");
    return close_popup(popup);
  }
  return null;
}
...
function find_current_popup() {
  ...
  if (active !== null && is_visible(active)) {
    let popup = find_enclosing_popup(active);
    if (popup !== null) {
      console.log("find_current_popup :: found parent");
      return popup;
    }
  }
  let popup = find_popup(document.body);
  if (popup !== null) {
    console.log("find_current_popup :: found child");
    return popup;
  }
  return null;
}
...
function is_reservation_popover(node) {
  return is_popover(node) && node.classList.contains("reservationPopover");
}
function is_popover(node) {
  return node.classList.contains("popover");
}
...
function find_reservation_popover() {
  return document.querySelector(".reservationPopover");
}

function find_spinner(root) {
  return root.querySelector(".spinner");
}
function no_spinner(root) {
  s = find_spinner(root);
  return isNull(s) || is_hidden(s);
}

function switch_to_price_tab(node = null) {
  console.log("switch_to_price_tab");
  if (node === null) {
    node = document.querySelector(".reservationPopover");
  }
...
async function edit_price(node = null) {
  ...
  click(btn);
  let price = await retryUntil(
    notNull,
    () => find_price_input(node),
    "input search"
  );
  // if (price !== null) {
  $(price).focus();
...
async function waitSelector(root, selector, label = null) {
  return await retryUntil(
    ...
    label !== null ? label : selector
  );
}

function expect_defined(x, label) {
  if (typeof x === "undefined") {
    throw "expected " + label + " to be defined";
  }
}

function is_liga(reservation) {
  return (
    getp(reservation, ["event", "clients", 0, "annotation"], "") ===
    "LIGA ŚRODA"
  );
}

async function set_liga_dorozliczenia() {
  let ligaIds = Object.entries(window.calendar.reservationsById)
  ...
  for (let ligaId of ligaIds) {
    ...
    await retryUntil(
      (x) => notNull(x) && no_spinner(x),
      find_reservation_popover,
      "popover search"
    );
    let details = await retryUntil(
      (x) =>
        defined(x) &&
        defined(x, "classList") &&
        !x.classList.contains("spinner"),
      () =>
        getp(window.calendar.reservationsById[ligaId], [
          "detailsPopover",
          "popoverContent",
          0,
        ])
    ...
    // await sleep(2000)
    let close_btn = await retryUntil(defined, () =>
      getp(window.calendar.reservationsById[ligaId], [
        "detailsPopover",
        "popoverContent",
        2,
