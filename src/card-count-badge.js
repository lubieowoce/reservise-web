...
export const compute_state = ({
  ...
  if (price_info === undefined) {
    return {
      type: "unknown",
      msg: `Unknown price (price_list_id: ${price_list_id})`,
    };
  } else {
    ...
    if (is_paid || (!is_paid && carnets === null)) {
      ...
      if (carnet_price_info === undefined) {
        return {
          type: "unknown",
          msg: `Unknown carnet (carnet_type: ${carnet_type})`,
        };
      }
...
export const render = (state) => {
  const make_badge = () =>
    $(`<span class="card-info-badge badge badge-pill"></span>`);
  let badge;
