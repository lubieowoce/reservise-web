...
export const fetch_event_price_info = (id) =>
  $ajax_promise({
    ...
    let [, price_list_id = null] =
      html.match(/data-price-list-id="(\d+)"/) || [];
    price_list_id =
    ...
    if (carnets === null) {
      ...
      if (client_id === null) {
        throw new Error(`no client id found for event ${id}`);
      }
      // client_id = Number(client_id)
      let reqs = Object.entries(carnets).map(([carnet_id, carnet]) => {
        return $ajax_promise({
          url: `/clients/carnet/create/${client_id}/`,
          dataType: "json",
          data: { carnet: carnet_id },
        }).then(({ html: carnet_html }) => {
          let carnet_type = (carnet_html.match(
            /<option value="(\d+)" price="[^"]+" days="[^"]+" resources="[^"]+" selected="selected">/
          ) || [null, null])[1];
          if (carnet_type === null) {
            throw new Error(
              `INTERNAL ERROR: no carnet type found for user ${client_id}, carnet ${carnet_id}`
            );
          }
      ...
      return Promise.all(reqs).then((new_carnets) => ({
        ...
        carnets: Object.fromEntries(new_carnets),
      }));
    }
  });

export const class_event_reservations = (event_id) => {
  return class_event_reservations_raw(event_id).then((text) =>
    parse_class_reservation($(text))
  );
};
...
const parse_class_reservation = (modal) => {
  const parse_client = (el) => {
    ...
    res["status"] = el.hasClass("rsv-active")
      ? "active"
      : el.hasClass("rsv-cancelled")
      ? "cancelled"
      : null;
    try {
      ...
      res["reservation_id"] = collapse
        .attr("href")
        .match(/#event-reservation-collapse-(\d+)/)[1];
      ...
      res["reservation_price"] =
        ...
        price2.length === 0
          ? $.trim(price.text())
            ? null
            : "standard"
          : price2.text().search(/-15[,.]00/) !== -1
          ? "multisport"
          : price2.text().search(/-20[,.]00/) !== -1
          ? "ok_gold"
          : null;
  ...
  return modal
    .find(".fitnessPanel")
    .toArray()
    .map((el) => parse_client($(el)));
};

export const add_class_reservation = ({
  client_id,
  event_id,
  price_list_id,
  price = "",
  ...
  return $ajax_promise({
    ...
    return $ajax_promise({
      type: "POST",
      url: "/events/create_class_reservation/",
      dataType: "json",
      data: {
...
export const create_client = (client_data) => {
  for (let param of ["last_name", "first_name"]) {
    if (!(param in client_data)) {
      throw new Error(
        `INTERNAL ERROR: Required named parameter '${param}' not present`
      );
    }
  }
  return _create_client(client_data).then((res) => {
    ...
    if (!match) {
      throw new Error(
        `INTERNAL ERROR: Could not recognize user ID from url: ${res.client_info_url}`
      );
    }
...
const _create_client = ({
  ...
  $ajax_promise({
    ...
    if (!token) {
      throw new Error(
        "INTERNAL ERROR: no csrf token found in response: " +
          JSON.stringify(data)
      );
    }
    return $ajax_promise({
...
const parseCreateClientFormErrors = (formText) => {
  ...
  const messages = [...form.querySelectorAll(".label.label-form-error")].map(
