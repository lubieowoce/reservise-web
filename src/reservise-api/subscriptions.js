...
export const unsafe_getClientSubscriptions = async ({clientId}) => {
    ...
    const res = (
        parseSubscriptionAccordion(el)
            .map((subscriptionInfo) => ({clientId, ...subscriptionInfo}))
    )
    el.remove()
    return res
}
...
const parseSubscriptionAccordion = (el) => {
    const subscriptionBoxes = (
        [...el.querySelectorAll('.subscription-during a[href^="#subscription-collapse-"]')]
    )
    return subscriptionBoxes.map(parseSubscriptionAnchor)
}


export const parseTimeFromSubscriptionAnchor = (box, { partial = false } = {}) => {
    const timeBox = box.querySelector('.subscription-dates')
    const timeStr = timeBox.innerText.replace(/\s+/g, ' ')
    let [match, start, end] = timeStr.match(
        /, (\d\d?:\d\d) - (\d\d?:\d\d),/
    ) || []
    if (match === undefined) {
    ...
    let duration
    try {
        duration = formatHour(parseHour(end) - parseHour(start), { padHour: false })
    } catch (e) {
        if (partial) { duration = null } else { throw e }
    }
...
export const parseSubscriptionAnchor = (box) => {
    const [,subscriptionId] = box.getAttribute('href').match(/^#subscription-collapse-(\d+)$/) || []
    const courtName = box.querySelector(':scope > strong').innerText.trim()
    const timeBox = box.querySelector('.subscription-dates')
    const timeStr = timeBox.innerText.replace(/\s+/g, ' ')
    let [match, day, start, end, periodStart, periodEnd] = timeStr.match(
    ...
    const duration = formatHour(parseHour(end) - parseHour(start), { padHour: false })
    ...
    try {
        ...
        throw new RangeError(`One or more invalid dates: ${periodStart}, ${periodEnd}`)
    }
    return {subscriptionId, courtName, day, start, duration, periodStart, periodEnd}
}
...
export const parseHour = (s) => {
    ...
    return hoursToMilliseconds(parseInt(hours)) + minutesToMilliseconds(parseInt(minutes))
}
