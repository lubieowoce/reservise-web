...
import { IconButton, GlyphIcon } from "./utils";
...
export const AddClient = ({
  ...
  const showAddOwner = !!(
    reservation_owner &&
    !user_entries.some(
      ({ user, card }) => card && user.id === reservation_owner.id
    )
  );
  const ownerName =
    reservation_owner.name ||
    reservation_owner.label.replace(/\s\([+0-9]+\)$/, "");

  return (
    <div className="custom-wrapper divWrapper setPaddingLR setPaddingTB">
      <h5 className="gray" style={{ marginBottom: "5px" }}>
        Gracze i karty zniżkowe
      </h5>
      <div className="divWrapper">
        ...
        {showAddOwner && (
          ...
          >
            <strong>+</strong> dodaj <strong>{ownerName}</strong> z kartą
            zniżkową
          </button>
        )}
      </div>
          ...
          >
            {showEditor && (
              <InlineClientEntryEditor
                ...
                hasCard={editorState.hasCard}
                onChange={(update) =>
                  setEditorState({ ...editorState, ...update })
                }
                onSubmit={(entry) => {
                  closeEditor();
            ...
            )}
            {
              ...
              >
                <span style={{ flexBasis: "100%" }}>
                  <a href={`/clients/c/${id}/`} target="blank">
                    {label}
                  </a>
                </span>
                {card && (
                  <span className="badge" style={{ flexBasis: "15%" }}>
                    MS
                  </span>
                )}
                <Popover
...
const makeClientEntryEditor =
  ({ SubmitButton, CancelButton }) =>
  ({
    userPicker: pickerState,
    hasCard,
    onChange = noop,
    onSubmit = noop,
    onCancel = noop,
    ...
    return (
      <div style={{ display: "flex" }}>
        ...
        {SubmitButton && (
          <SubmitButton onClick={onSubmitClicked} disabled={!canSubmit} />
        )}
...
const InlineClientEntryEditor = makeClientEntryEditor({
  SubmitButton: (props) => (
    <button {...props} className="btn btn-link" title="Zapisz">
      <GlyphIcon name="ok" />
    </button>
  ),
  CancelButton: (props) => (
    <button {...props} className="btn btn-link" title="Anuluj">
      ...
      {state.type === PickerState.CREATE && (
        ...
        >
          <ClientEditor
            {...state.data}
            onChange={(update) =>
              onChange({ ...state, data: { ...state.data, ...update } })
            }
            onCancel={() => {
              onChange({
                type: PickerState.SEARCH,
                search: state.originalSearch || "",
              });
              transferFocus();
            }}
            containerProps={{
  ...
  return (
    <div {...containerProps}>
      <div style={{ padding: "0.5em" }}>
        <div style={{ display: "flex" }}>
          <label {...labelProps}>
            <span {...labelTextProps}>{FIELD_INFO["firstName"].label}</span>{" "}
            <input {...textFieldProps("firstName")} />{" "}
          </label>
          <span style={{ width: "0.3em" }} />
          <label {...labelProps}>
            <span {...labelTextProps}>{FIELD_INFO["lastName"].label}</span>{" "}
            <input {...textFieldProps("lastName")} />{" "}
          </label>
        </div>
        <label {...labelProps}>
          <span {...labelTextProps}>{FIELD_INFO["phone"].label}</span>{" "}
          <input {...textFieldProps("phone")} />{" "}
        </label>
        <label {...labelProps}>
          <span {...labelTextProps}>{FIELD_INFO["email"].label}</span>{" "}
          <input {...textFieldProps("email")} />{" "}
        </label>
        <label {...labelProps}>
          <span {...labelTextProps}>{FIELD_INFO["annotation"].label}</span>{" "}
          <textarea
            {...textFieldProps("annotation", {
