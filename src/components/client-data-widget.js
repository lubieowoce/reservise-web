...
import { GameResults, replaceUser, removeResultsWithUser } from './game-results-widget'
...
export const ClientData = ({
    ...
    const users = useMemo(() =>
        usersFromEntries({reservation_owner, user_entries})
            .map((u) => u.name ? u : {...u, name: u.label.replace(/\s\([+0-9]+\)/, '')}),
    ...
    const onRemoveEntry = (index) => {
        ...
        if (!newUserIds.includes(removedUserId)) {
            changed.game_results = removeResultsWithUser(
                game_results,
                {id: removedUserId}
            )
        }
    ...
    const onModifyEntry = (index, entry) => {
        ...
        if (oldUserId !== newUserId) {
            ...
            if (!newUserIds.includes(oldUserId)) {
                changed.game_results = replaceUser(
                    game_results,
                    {oldId: oldUserId, newId: newUserId}
                )
            }
        }
